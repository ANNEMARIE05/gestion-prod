...
export class ProductionComponent {
  ...
  @Input() set typeInput(val: ProductionType) {
    if (val) {
      this.type.set(val);
      this.meta.set(PAGE_META[val]);
      this.typeInputSet = true;
    }
  }
  ...
  constructor(private router: Router, private route: ActivatedRoute) {
    this.route.data.subscribe(data => {
      ...
      // Ne pas écraser si typeInput a déjà été appliqué
      if (!this.typeInputSet) {
        this.type.set(t);
