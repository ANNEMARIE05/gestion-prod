...
export class ProfileFormPageComponent implements OnInit {
  ...
  readonly allMenus = computed(() =>
    flattenMenuItems(this.settings.allMenus())
      .filter((m) => m.active)
  ...
  readonly availableMenus = computed(() => {
    ...
    const used = new Set(
      this.habilitations()
        .map((h, index) => (index === editing ? null : h.menuId))
        .filter((id): id is string => !!id),
    );
    return this.allMenus().filter((m) => !used.has(m.id));
  });
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.settings.loadProfileById(id).subscribe((p) => {
        ...
        this.habilitations.set(this.profileToHabilitations(p));
      });
    }
  }
  ...
  private profileToHabilitations(profile: AuthorizationProfile): ProfileHabilitation[] {
    return profile.visibleMenuIds.map((menuId) => ({
      menuId,
      menuLabel: this.settings.getMenuById(menuId)?.label ?? menuId,
      actionIds: [...(profile.allowedActionsByMenu[menuId] ?? [])],
    }));
  }
