...
export class SettingsService {
  ...
  refreshSettings(force = false): void {
    this.loadFromApi(force).subscribe();
  }

  loadFromApi(force = false): Observable<void> {
  ...
  deleteMenuItem(id: string): Observable<void> {
    ...
    return this.menusApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteAction(actionId: string): Observable<void> {
    ...
    return this.deleteActionViaApi(action).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  persistProfile(profile: AuthorizationProfile, isCreate: boolean): Observable<void> {
    ...
    return request$.pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteProfile(id: string): Observable<void> {
    ...
    return this.profilsApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteUser(id: string): Observable<void> {
    ...
    return this.ressourcesApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  persistMenu(
    ...
    return request$.pipe(
      switchMap((resp) => {
        ...
        return this.loadFromApi(true).pipe(catchError(() => of(undefined)));
      }),
    );
  }
  ...
  deleteEntity(id: string): Observable<void> {
    ...
    return this.entitesApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteSpecialty(id: string): Observable<void> {
    ...
    return this.specialitesApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteApplication(id: string): Observable<void> {
    ...
    return this.applicationsApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
  ...
  deleteJalon(id: string): Observable<void> {
    ...
    return this.jalonsApi.delete(apiId).pipe(switchMap(() => this.loadFromApi(true)));
  }
...
function profileToApiPayload(profile: AuthorizationProfile) {
  const permissionRequests = profile.visibleMenuIds
    .map((menuId) => {
      ...
      const actionIds = (profile.allowedActionsByMenu[menuId] ?? []).map(String);
      const seen = new Set<string>();
      const uniqueActionIds = actionIds.filter((id) => {
