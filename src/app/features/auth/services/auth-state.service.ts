...
export class AuthStateService {
  private stateSignal = signal(authStore.getState());

  constructor() {
    // Synchroniser les changements du store Zustand avec le signal Angular
  ...
  setAuth(auth: any) {
    ...
    if (userInfos) {
      authStore.getState().setUserInfos(this.normalizeUserInfos(userInfos, auth));
    }
  }

  setUserInfos(userInfos: any) {
    if (!userInfos) return;
    authStore.getState().setUserInfos(this.normalizeUserInfos(userInfos, userInfos));
  }

  userInfos(): any | null {
    return this.stateSignal().userInfos;
  }
